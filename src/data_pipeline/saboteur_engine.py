...
class SaboteurOutput(BaseModel):
    ...
    error_type: Literal["Entity Swap", "Numerical Drift", "Logical Negation"]

# ---------------------------------------------------------------------------
# Prompts
# ---------------------------------------------------------------------------
SYSTEM_PROMPT = """You are the Saboteur, an expert in generating adversarial training data.
Your task is to take a factual context and generate a JSON object with a mutated, incorrect answer.

You must choose exactly ONE of the following error types to inject:
...
3. "Logical Negation": Flip the meaning of a key sentence (e.g., changing "is" to "is not", or "increased" to "decreased").
...
Your output MUST be valid JSON matching the following schema:
{
  ...
  "sabotaged_answer": "The answer to the question, but with the selected error injected",
  "error_type": "The exact name of the type of error injected (must be 'Entity Swap', 'Numerical Drift', or 'Logical Negation')"
}
            ...
            try:
                response = await client.chat.completions.create(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": f"Context to sabotage:\n{context}"}
                    ],
...
async def generate_sabotage_data(contexts: list[str], output_path: str, concurrency_limit: int = 5):
    ...
    if not api_key:
        logger.error("GROQ_API_KEY environment variable is not set. Please set it before running.")
        return
    ...
    tasks = [
        generate_sabotage_for_context(client, context, semaphore)
...
if __name__ == "__main__":
    ...
    # pip install groq pydantic pandas datasets
